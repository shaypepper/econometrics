...
|-----------------------|-------------------------|-------------------------|-----------------|
                        |  Smoked 100+ cigaretts  | Smoked <100 cigarettes  | Marginal
| GENERAL HEALTH        |         |         |              
...
3. (10 points) Start with some basic statistics: how does the tendency to be overweight vary among educational groups? Are these differences statistically significant? Discuss.
...
| Age2*Female        | 0.001       | 0.0004
    ...
    b. 
    * Male, 35: $BMI= 18.968 + 0.523(35) + -0.006(35^2) + 1.723(0) - 0.122(0) + 0.001(0) = `r 18.968 + 0.523 * 35 - 0.006 * (35 ** 2)`$
    * Female, 35: $BMI= 18.968 + 0.523(0) + -0.006(0) + 1.723(1) - 0.122(35) + 0.001(35^2) = `r 18.968 + 0.523 * 0 - 0.006 * 0 + 1.723 * 1 + -0.122 * 35 + 0.001 * (35 ** 2)`$
    ```{r}
    mensBMI <- function (ma){
      18.968 + 0.523 * (ma) - 0.006 * (ma ** 2) 
    }
    mensBMI(35)
    mensBMI(36)
    # So 36 for men
    ...
    womensBMI <- function (wa){
      18.968 + 0.523 * (wa) - 0.006 * (wa**2) + 1.723 * 1 + -0.122 * wa + 0.001 * (wa ** 2)
    }
    womensBMI(39)
    womensBMI(40)
    womensBMI(41)
    womensBMI(42)
    # Women seem to peak at 40, and never hit an expected 30. 
    ```
...
mensBMI(44)
mensBMI(45)
# Men peak at age 44 with level 30.364


# From previous answer, women peak at 40 with level 28.731
```
    d. [BMI is bullshit, so emphatically no. No model here seems reasonable. (Click here for an NPR article on the subject)](https://www.npr.org/templates/story/story.php?storyId=106268439?storyId=106268439)


2. 
b.
$$
\frac{1}{1 + e^{-2.355 + 0.197  age - 0.0020  age^2}} = 
\frac{1}{1 + e^{-2.355 + 0.197  (35) - 0.0020  (35^2)}} \approx 32.36\%
$$

c.   
    ```{r}
probMaleOverweight <- function(age){
  1/ (1 + exp(-(-2.355 +  0.197 * age - 0.0020 * (age ** 2))))
}
probFemaleOverweight <- function(age){
  1/ (1 + exp(-(-2.355 +  0.197 * age - 0.0020 * (age ** 2) + 1.360  + -0.100 * age + 0.0011 * (age**2))))
}

probMaleOverweight(48)
probMaleOverweight(49)
probMaleOverweight(50)
probMaleOverweight(53)
probMaleOverweight(55)
# Men are most likely to be overweight at 49


probFemaleOverweight(53)
probFemaleOverweight(54)
probFemaleOverweight(55)
# 54 is the peak for women


```
### Age gaps for married people
Here's an idea of how you might solve this
With dplyr:
```{r eval = FALSE}
...
by_educ <- new_df %>%
  group_by(EDU) %>%
  summarize(spouse_age = mean(AGE), head_age = mean(h_age), mean_gap = mean(abs_gap))
...
```{r eval = FALSE}
older <- mean(abs(data$gap[AGE > 50]))
edu_2 <- mean(abs(data$gap[EDU = 2]))
